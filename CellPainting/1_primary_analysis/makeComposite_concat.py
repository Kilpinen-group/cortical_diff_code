...
outputRescaled = outputFolder + "/outRescaled/"
...
if not os.path.exists(outputRescaled): os.mkdir(outputRescaled)
...
for well in filePerField_Well:
    for field in filePerField_Well[well]:
        ...
        img_basename = well+field
        argsForMakeCompositeImg[well + field] = (paths, outputComposite, outputRescaled, img_basename)
        fieldInt = int(field[3:])
        if not well in filePerWell: filePerWell[well]=[0]*nFields
        filePerWell[well][imagePerField[fieldInt-1]]=outputComposite+img_basename+".png"
...
with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
    ...
    for wellField in argsForMakeCompositeImg:
        future = executor.submit(makeCompoUtils.makeCompositeImg, *argsForMakeCompositeImg[wellField])
...
def mergeField(well):
    ...
    makeCompoUtils.overwriteImage(outputWellPics+"/"+well+".png", output)
    outputSmall=cv2.resize(output, (0,0), fx=0.25, fy=0.25)
    makeCompoUtils.overwriteImage(outPutWellSmall+"/"+well+".png", outputSmall)
